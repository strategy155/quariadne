...
class LpRouterLayered:
    ...
    def _convert_mapping_to_fixed_edges(
        self,
        layer_end_mapping: dict[
            quariadne.circuit.LogicalQubit, quariadne.circuit.PhysicalQubit
        ],
        first_layer_ops_local: list[int],
        subcircuit_operations: list[quariadne.circuit.QuantumOperation],
    ) -> dict[int, int] | None:
        """Convert qubit positions to fixed edge constraints for first-layer ops.

        .. deprecated::
            This method is deprecated in favour of using `initial_position_mapping`
            directly in LpRouterUnified. The edge-based approach over-constrains
            by pinning BOTH qubits simultaneously, which can cause LP infeasibility.
            Use the position-based approach in `_add_initial_position_constraints()`
            instead.

        This is the key constraint propagation mechanism. Given where qubits
        ended up after the previous layer (layer_end_mapping), we constrain
        the first operations of the next subcircuit to respect these positions.

        For each operation in the first layer of the subcircuit:
        1. Look up both qubits' required positions from layer_end_mapping
        2. The operation must execute on the edge connecting these positions
        3. If that edge exists, fix the operation to it
        4. If the edge doesn't exist (qubits not adjacent), skip the constraint
           and let the LP find optimal swaps to bring them together

        Args:
            layer_end_mapping: Qubit positions from previous layer's end.
            first_layer_ops_local: Local indices of first layer operations.
            subcircuit_operations: Operations list for the subcircuit.

        Returns:
            Dict mapping local operation index to edge index, or None if
            no constraints could be determined.
        """
        fixed_edges: dict[int, int] = {}
        has_unfixable_operation = False

        for local_op_idx in first_layer_ops_local:
            operation = subcircuit_operations[local_op_idx]
            left_qubit, right_qubit = operation.qubits_participating

            # Get required physical positions from previous layer's mapping
            left_pos = layer_end_mapping.get(left_qubit)
            right_pos = layer_end_mapping.get(right_qubit)

            # Skip if qubit not in previous mapping (e.g., first appearance,
            # dummy qubit, or qubit that wasn't involved in previous layers)
            if left_pos is None or right_pos is None:
                continue

            # The operation (left_qubit, right_qubit) must execute on edge
            # (left_pos, right_pos) to maintain correct qubit positions.
            # IMPORTANT: Edge orientation matters! For operation (A, B) on edge (u, v):
            #   - A sits at u (edge[0])
            #   - B sits at v (edge[1])
            # So we MUST use edge (left_pos, right_pos), NOT (right_pos, left_pos).
            # Using the reversed edge would swap qubit positions incorrectly.
            required_edge: PhysicalEdge = (left_pos, right_pos)
            edge_idx = self.edge_to_index.get(required_edge)

            if edge_idx is not None:
                # Edge exists with correct orientation - fix operation to this edge
                fixed_edges[local_op_idx] = edge_idx
            else:
                # Edge doesn't exist in required orientation.
                # We CANNOT use the reversed edge because that would put qubits
                # at wrong positions. Mark as unfixable.
                has_unfixable_operation = True
                _logger.debug(
                    "Op %d: edge (%s, %s) not in coupling graph, skipping all constraints",
                    local_op_idx,
                    left_pos,
                    right_pos,
                )

        # IMPORTANT: If ANY operation couldn't be fixed, return None to avoid
        # partial constraints that may cause infeasibility. When some operations
        # are fixed and others aren't, shared qubits can create conflicts.
        if has_unfixable_operation:
            _logger.debug(
                "Some operations unfixable, skipping all fixed_edges constraints"
            )
            return None

        return fixed_edges if fixed_edges else None
